...
rep <- full_join(testSiteTrunc, sitesTrunc, by = "site")
repFilter <- rep %>% select(site, lat, long, Lat_Y, Long_X) %>% filter(lat == Lat_Y)

repLeft <- left_join(testSiteTrunc, sitesTrunc, by = 'site')
