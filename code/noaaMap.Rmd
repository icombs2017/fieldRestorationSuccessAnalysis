...
pacman::p_load("cowplot", "flextable", "ggrepel", "ggspatial", "lubridate", "officer", "paletteer", "patchwork", "rgdal", "rnaturalearth", "sf", "tidyverse", "gdata", "rgeos", "ggpubr")
...
```{r, map data}
sites = read.xls("../data/outplantMetadata.xlsx", header =T)
...
mainKeys <- ggplot() +
  ...
  coord_sf(xlim = c(-82.1688, -80.9603), ylim = c(24.3395,  24.8191)) +
  geom_point(data = sites, aes(x = Long_X, y = Lat_Y, fill = Site_ID), size = 5) +
...
noaaSites <- ggplot() +
  ...
  geom_point(data = sites, aes(x = Long_X, y = Lat_Y, fill = Site_ID, shape = 0, color = Site_ID), size = 2, alpha = 1) +
